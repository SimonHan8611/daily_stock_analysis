...
describe("ChatPage", () => {
  ...
  it("opens and closes the mobile history drawer through the overlay", async () => {
    ...
    expect(container.querySelector(".page-drawer-overlay")).toBeInTheDocument();
    expect(
      screen.getAllByRole("heading", { name: "历史对话" }).length,
    ).toBeGreaterThan(0);
    ...
    await waitFor(() => {
      expect(
        container.querySelector(".page-drawer-overlay"),
      ).not.toBeInTheDocument();
    });
  });
  ...
  it("filters quick questions by available skills and sends the selected suggestion with its mapped skill", async () => {
    mockGetSkills.mockResolvedValue({
      skills: [{ id: "bull_trend", name: "趋势分析", description: "测试技能" }],
      default_skill_id: "bull_trend",
    });
    ...
    const quickQuestion = await screen.findByRole("button", {
      name: "分析比亚迪趋势",
    });

    expect(
      screen.queryByRole("button", { name: "用缠论分析茅台" }),
    ).not.toBeInTheDocument();
  ...
  it("uses the currently selected skill when sending a manually entered question", async () => {
    mockGetSkills.mockResolvedValue({
      skills: [
        { id: "bull_trend", name: "趋势分析", description: "测试技能" },
        { id: "wave_theory", name: "波浪理论", description: "波段节奏" },
      ],
      default_skill_id: "bull_trend",
    });
    ...
    fireEvent.click(await screen.findByRole("radio", { name: "波浪理论" }));
  ...
  it("renders the loading status bubble with the current progress stage", async () => {
    ...
    expect(await screen.findByText("市场数据...")).toBeInTheDocument();
